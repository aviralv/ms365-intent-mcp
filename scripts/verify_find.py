...
Calls _find_impl against a live Graph connection and prints
the structured response fields plus the rendered markdown.

Usage:
    uv run python scripts/verify_find.py "search term"
    uv run python scripts/verify_find.py "budget" email
    uv run python scripts/verify_find.py "report" file
"""
...
async def run(query: str, entity_type: str | None = None) -> None:
    ...
    async with GraphClient(config.graph_base_url, auth.get_access_token) as client:
        ...
        _line()
        print(f"find — query={query!r}, entity_type={entity_type!r}")
        _line()

        result = await _find_impl(ctx, payload)
