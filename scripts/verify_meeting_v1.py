...
Calls _meeting_v1_impl against a live Graph connection and prints
the structured response fields plus the rendered markdown.

Usage:
    uv run python scripts/verify_meeting_v1.py
    uv run python scripts/verify_meeting_v1.py "next"
    uv run python scripts/verify_meeting_v1.py "Refinement"
"""
...
async def run(identifier: str) -> None:
    ...
    async with GraphClient(config.graph_base_url, auth.get_access_token) as client:
        ...
        _line()
        print(f"meeting_v1 — identifier={identifier!r}")
        _line()

        result = await _meeting_v1_impl(ctx, payload)
        ...
        print("✅ verify_meeting_v1 PASSED")
